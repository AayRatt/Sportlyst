...
export default function ChatFriends({ navigation }) {
    ...
    const getFriends = async () => {
        try {
            const friendsDb = collection(db, 'userProfiles', auth.currentUser.uid, 'friends')
                        ...
                        return (
                            <TouchableOpacity onPress={ () => navigation.navigate('Chat', { 
                                ...
                                <View className="flex-row items-center mt-5 pl-3">
                                    <Image
                                        source={rowData.item.friendProfile?.image ? { uri: rowData.item.friendProfile.image } : profileIcon}
                                        className="w-12 h-12 rounded-full"
                                    />
                                    <Text
                                        className="font-urbanist text-1xl mr-3"
                                    >{` ${rowData.item.friendProfile.firstName} ${rowData.item.friendProfile.lastName}`}</Text>
